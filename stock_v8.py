    ...
    if data[3] == "I" :
        ...
        if (round(((int(current_Amt.replace(",",""))*int(data[6])) - int(data[7])) / int(data[7]) * 100 ,2) >= stock_constant.SELL_UP_RATE) :
            ...
            if(amt <= int(current_Amt.replace(",",""))*int(data[6])):
                upRateMaintainStock(data,current_Amt,VERSION_META_TABLE,VERSION_TABLE,'▲ up rate maintain : '+item_text)
            else:  
...
def purchaseStock(stockData,current_Amt,VERSION_META_TABLE,VERSION_TABLE):
    if dpstock.get(stockData[1]) == None:
        ...
        tmpIdx = tmpIdx.replace("stock_v8_","").replace("_meta","")
...
# up rate maintain stock
def upRateMaintainStock(stockData,current_Amt,VERSION_META_TABLE,VERSION_TABLE,msg):
    chg_dttm = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def main_process():   
    ...
    for idx, data in enumerate(stock_constant.CRAWLING_TARGET):  
        if data['skip'] == False:
            log('--- Run ---['+str(idx)+'] '+data['title'],"N")
            RUN_CMD_INDEX = idx
            VERSION_META_TABLE = 'stock_v8_'+str(idx)+'_meta'
            VERSION_TABLE = 'stock_v8_'+str(idx)
