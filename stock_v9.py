...
# - stock_v9테이블 status 컬럼 값이 I 인 종목 대상
...
#   - sqlite3 stock.db
# 
#   create table stock_v9_x_meta (current_amt text);
#   insert into stock_v9_x_meta (current_amt) values ('500000');
#
#   create table stock_v9_x (id integer primary key autoincrement, code text, item text, status text
...
def getStocInfoData(data,status,VERSION_META_TABLE,VERSION_TABLE):
    ...
    if data[3] == "I" :
        ...
        item_sub_text +=" 수익금액 : "+fill_str_space(str(format(int(current_Amt.replace(",",""))*int(data[6]) - int(data[7]),',')))

        log(item_text,"N")
        processing = True

        if (round(((int(current_Amt.replace(",",""))*int(data[6])) - int(data[7])) / int(data[7]) * 100 ,2) >= SELL_UP_RATE) :
            # SELL_UP_RATE 이상 이면 계속 유지 하면서 기존 상승된 값 보다 낮아진 경우 매도 처리 
            amt = 0 
            if(data[9] == None):
...
def purchaseStock(stockData,current_Amt,VERSION_META_TABLE,VERSION_TABLE):
    if dpstock.get(stockData[1]) == None:
        ...
        log(fill_str_space('['+CRAWLING_TARGET[int(tmpIdx)]['title']+']',25)+' ☆ purchase :'+item_text,"Y")    
...
def sellStock(stockData,current_Amt,VERSION_META_TABLE,VERSION_TABLE,msg):
    ...
    tmpIdx = VERSION_META_TABLE
    tmpIdx = tmpIdx.replace("stock_v9_","").replace("_meta","")
    log(fill_str_space('['+CRAWLING_TARGET[int(tmpIdx)]['title']+']',25)+' '+msg,"Y")
