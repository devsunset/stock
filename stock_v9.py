...
def getStocInfoItemList(RUN_CMD_INDEX,excludeData):
    ...
    for idx, data in enumerate(infoData):
        ...
        # 현재가가 stock_constant.CURRENT_AMOUNT_MAX 이하
        # 등락율이 상승인 종목만 선택
        ...
        if downcheck <= stock_constant.SELL_DOWN_CNT and dpstock.get(codeData[idx].replace("/item/main.nhn?code=","")) == None and codeData[idx].replace("/item/main.nhn?code=","") not in excludeData and int(data[stock_constant.CRAWLING_TARGET[RUN_CMD_INDEX]['current_amtIdx']]) <= stock_constant.CURRENT_AMOUNT_MAX and data[stock_constant.CRAWLING_TARGET[RUN_CMD_INDEX]['uprateIdx']][0] == "+":
            # print(data)
...
def getStocInfoData(data,status,VERSION_META_TABLE,VERSION_TABLE):
    ...
    if data[3] == "I" :
        ...
        item_sub_text = " 종목 : "+fill_str_space(data[2],25) 
        item_sub_text +=" 구매금액 : "+fill_str_space(str(format(int(data[7]), ',')))
        item_sub_text +=" 현재금액 : "+fill_str_space(str(format(int(current_Amt.replace(",",""))*int(data[6]),',')))
        item_sub_text +=" 등락율 : "+ fill_str_space(str(round(((int(current_Amt.replace(",",""))*int(data[6])) - int(data[7])) / int(data[7]) * 100 ,2))+"%")
        item_sub_text +=" 수익금액 : "+fill_str_space(str(format(int((now_amt - commission_amt - tax_amt )- int(data[7])),',')))

        log(item_text,"N")
        processing = True

        if (round(((int(current_Amt.replace(",",""))*int(data[6])) - int(data[7])) / int(data[7]) * 100 ,2) >= stock_constant.SELL_UP_RATE) :
            # stock_constant.SELL_UP_RATE 이상 이면 계속 유지 하면서 기존 상승된 값 보다 낮아진 경우 매도 처리 
            amt = 0 
            if(data[9] == None):
                amt = int(current_Amt.replace(",",""))*int(data[6])
            else:
                amt = int(data[9]) 
            
            if(amt <= int(current_Amt.replace(",",""))*int(data[6])):
                sellPassStock(data,current_Amt,VERSION_META_TABLE,VERSION_TABLE,'▲ sell pass : '+item_sub_text)
            else:  
                sellStock(data,current_Amt,VERSION_META_TABLE,VERSION_TABLE,'▲ sell : '+item_sub_text)
                processing = False
        ...
        if processing and status == "CLOSE":
            sellStock(data,current_Amt,VERSION_META_TABLE,VERSION_TABLE,' ★ Closed sell : '+item_sub_text)
        ...
        sellStock(data,current_Amt,VERSION_META_TABLE,VERSION_TABLE,' ★ Forced sell : '+item_sub_text)
...
def purchaseStock(stockData,current_Amt,VERSION_META_TABLE,VERSION_TABLE):
    if dpstock.get(stockData[1]) == None:
        ...
        tmpIdx = tmpIdx.replace("stock_v9_","").replace("_meta","")

        item_text = " 종목 : "+fill_str_space(stockData[2],25) 
        item_text +=" 구매가 : "+fill_str_space(format(int(current_Amt), ','))
        item_text +=" 구매수 : "+fill_str_space(str(format(purchase_count, ',')))
        item_text +=" 구매금액 : "+fill_str_space(str(format(purchase_amt, ',')))

        log(fill_str_space('['+stock_constant.CRAWLING_TARGET[int(tmpIdx)]['title']+']',25)+' ☆ purchase :'+item_text,"Y")    

        dpstock[stockData[1]] = "P"
...
def sellStock(stockData,current_Amt,VERSION_META_TABLE,VERSION_TABLE,msg):
    ...
    tmpIdx = VERSION_META_TABLE
    tmpIdx = tmpIdx.replace("stock_v9_","").replace("_meta","")
    log(fill_str_space('['+stock_constant.CRAWLING_TARGET[int(tmpIdx)]['title']+']',25)+' '+msg,"Y")

    if dpstock.get(stockData[1]) != None:
        del dpstock[stockData[1]]

# sell pass stock
def sellPassStock(stockData,current_Amt,VERSION_META_TABLE,VERSION_TABLE,msg):
    chg_dttm = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    up_amt = int(current_Amt.replace(",",""))*int(stockData[6])
    sql = "update "+VERSION_TABLE+" set up_amt = ? ,chg_dttm = ? where id = ?"
    sqlParam =  (up_amt, chg_dttm, stockData[0])
    executeDB(sql,sqlParam)
...
def main_process():   
    ...
    for idx, data in enumerate(stock_constant.CRAWLING_TARGET):  
        if data['skip'] == False:
            log('--- Run ---['+str(idx)+'] '+data['title'],"P")
            RUN_CMD_INDEX = idx
            VERSION_META_TABLE = 'stock_v9_'+str(idx)+'_meta'
            VERSION_TABLE = 'stock_v9_'+str(idx)
